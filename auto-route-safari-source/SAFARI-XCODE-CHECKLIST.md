...
2. In the generated Xcode project, select the **Auto Route** app target:
   - choose your signing team;
   - add the **App Groups** capability with `group.com.autoroute.app`;
   - add **Incoming Connections (Server)** under App Sandbox;
   - set its Code Signing Entitlements file to `Native/AutoRoute.entitlements`, or copy those keys into the generated app entitlements file.
3. Select the **Auto Route Extension** target:
...
4. Confirm the generated extension bundle identifier is `com.autoroute.app.Extension`. If Xcode chose another suffix, update that identifier in `Native/AppDelegate.swift` before copying/building.
